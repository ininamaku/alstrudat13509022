...
/* Mengirimkan nilai rata-rata info(P) */
	...
	return ((double)sum / (double)NbElmt(L));
}
...
/* Lout adalah list kosong jika ada alokasi elemen yang gagal */
	...
	while (akses != Nil) {
		...
		InsertLast(Lout, Alokasi(info(akses)));
		akses = next(akses);
	}
	if (gagal) {
		DelAll(Lout);
...
/* dan semua elemen yang pernah dialokasi didealokasi */
	...
	CpAlokList(L1,L3);
	CpAlokList(L2,&L4);
	if ((First(*L3) != Nil) && (First(L4)!=Nil)) {
		akses = First(*L3);
		while (next(akses) != Nil) akses = next(akses);
		next(akses) = First(L4);
	}
}
